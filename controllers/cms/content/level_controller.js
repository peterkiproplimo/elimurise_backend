...
router.get('/', async (req, res) => {
  try {
    ...
    const totalLevels = await Level.countDocuments(query);
    const total_pages = Math.ceil(totalLevels / limit);

    const levelsList = await Level.find(query).sort({createdAt: -1}).skip(skip).limit(limit);
